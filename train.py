...
def train():
    ...
    train_dataset = SaltDataset(opt.data_path, transform=x_transform, target_transform=y_transform)
    ...
    for epoch in range(start_epoch + 1, opt.epochs):
        ...
        for inputs, masks in pbar:
            ...
            pbar.set_description("Epoch:[%d/ %d] Average running loss %.7f" % (epoch,
                                 opt.epochs, (running_loss / dataset_size)))
            step += 1
